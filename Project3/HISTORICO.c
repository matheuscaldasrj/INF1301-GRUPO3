...
HIS_tpCondRet HIS_verificaHistorico(HIS_tpHistorico * pHistorico ) {
	int totalErrors = 0;
	int error = 0;
	List * list = pHistorico->disciplinasCursadas;

	verificaCursorNull(list,&error);
	if(error == 1){
		return HIS_ErroNaEstruturaDisciplinasCursadas;
	}
...
HIS_tpCondRet HIS_deturpaListaDeHistorico (HIS_tpHistorico * pHistorico , int acaoDeDeturpacao )
{

	if( acaoDeDeturpacao == 1 )
	{
		// LISTA FICA NULL
		deturpaListaNull(&(pHistorico->disciplinasCursadas));
	} 	
	else if( acaoDeDeturpacao == 2 )
	{
		// CURSOR fica null
		deturpaCursorNull((pHistorico->disciplinasCursadas));
	} else if (acaoDeDeturpacao == 3) {

		// LAST FICA NULO
		deturpaLastNull((pHistorico->disciplinasCursadas));
	} else if(acaoDeDeturpacao == 4) {
		// ANTERIOR FUCA NULLO
		deturpapAnteriorNull((pHistorico->disciplinasCursadas));
	} else if(acaoDeDeturpacao == 5) {
		// FIRST FICA NULLO
		deturpaFirstNull((pHistorico->disciplinasCursadas));
	}  else if(acaoDeDeturpacao == 6) {
		// VALOR CORRENTE FICA NULLO
		deturpaValorCorrenteNull((pHistorico->disciplinasCursadas));
	}
